...
class TestVisitSensor(unittest.TestCase):
    ...
    def test_on_special_dates(self):
        for month, day in [(1, 1), (5, 1), (12, 25)]:
            with self.subTest(i=month, j=day):
                visit_sensor = VisitSensor(1200, 300)
                visit_count = visit_sensor.simulate_visit_count(date(2023, month, day))
                self.assertEqual(visit_count, -1)
